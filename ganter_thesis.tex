...
% phi[r] should maybe start as phi[-n] s.t. we are grabbing
% "future" samples just like the window, and the final value
% is evaluated as phi[+r]

% we could have phi[r] = F0[n]r for each new window
% this might mess up phase stuff but wouldn't matter for
% magnitude only

% we could talk about the implications of phi[r] when F0 is
% constant i.e. phi[n] = F0n in which case this is an STFT at
% special frequencies, then talk about the interpretation when
% F0 is not constant
