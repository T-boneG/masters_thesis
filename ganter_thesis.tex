...
The other components come from interactions with the unwanted harmonics that we failed to completely filter out.  We have a magenta term at $\pm 2 F_0$ from the adjacent harmonics (blue and red) convolving with each other.  We have a cyan and yellow term at $\pm F_0$ from either adjacent harmonic interacting with the desired harmonic (green).  We then have a mixture at baseband due to each harmonic convolving with itself.
...
Recalling equation~\ref{eq:downshift_radian_frequency}, if $F_{ds,k} = (k+1)F_0$ then, $w_{k,k} = 0$ and the coherent gain is simply the DC signal gain of the filter.

\begin{align}
G_k = \Big| H_k(0) \Big| = \sum_n h_k[n]
\end{align}

We may further simplify this by designing our filter such that $\sum_n h_k[n] = 1$.  Of course, our downshift frequency won't be ideal in real systems.  Factors to consider include the quantization of downshift frequencies and the accuracy of $F_0$ estimation.
