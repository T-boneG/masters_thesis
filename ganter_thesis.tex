...
Why is the $\frac{1}{k}$ term necessary?  Consider an example where our true pitch estimate is actually $f_{0,ground truth} = \tilde{f}_0 + f_{err}$.  So,
...
2) $f_0[n]$ is roughly constant within a time window of $\frac{N}{f_s}$ seconds
...
For coherent gain the worst case us roughly $G_k \leq -3$dB.  From this result it doesn't appear to be an overly critical design consideration.
